...
export const RoleGate = ({
	...
	// If context not mounted yet, fallback prevents hydration mismatch
	if (!auth) return <>{fallback ?? null}</>;
	if (auth.loading) return <>{fallback ?? null}</>;
	if (!auth.user) return <>{fallback ?? null}</>;

	const claimRole = (auth.customClaims?.role as string) || "faculty";
	if (roles.includes(claimRole as AppRole)) return <>{children}</>;
